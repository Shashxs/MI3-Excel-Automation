...
function createWindow() {
  ...
  ipcMain.handle('text-copied-2', async (event, selectedOption) => {
    ...
    const excelFileName = `${username}_MI3_Agent_Cases.xlsx`;
...
//   const homeDir = os.homedir(); // Get the user's home directory
//   const excelFileName = `${username}_MI3_Agent_Cases.xlsx`;
//   const excelFilePath = path.join(homeDir, 'OneDrive - Deloitte (O365D)', 'MI3', excelFileName);
//   const workbook = new exceljs.Workbook();
//   try {
//     await workbook.xlsx.readFile(excelFilePath);
//   } catch (error) {
...
async function getCurrentTimestamp(userID) {
  const username = os.userInfo().username;
  const homeDir = os.homedir(); // Get the user's home directory
  const excelFileName = `${username}_MI3_Agent_Cases.xlsx`;
  const excelFilePath = path.join(homeDir, 'OneDrive - Deloitte (O365D)', 'test', excelFileName);
  ...
  const targetColumns1 = ['UserID'];
  targetColumns1.forEach((columnName, columnIndex) => {
    sheet.getCell(startTimeRow, columnIndex + 11).value = userID;
  });
...
let nextRow=1;
  ...
  console.log('End Time:', sheet.getCell(nextRow, 7).value);
